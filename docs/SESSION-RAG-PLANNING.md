...
数据链路是：文本导入 → 换行规范化 → 重叠分块 → BM25 / embedding 混合检索 → 注入原文 → 生成回答 → 展示实际引用。Embedding 可选启用，配置方法见 [Embedding 与向量检索](EMBEDDING-RETRIEVAL.md)。
...
- 默认检索使用英文词和中文双字词组；配置 embedding 后增加余弦语义召回，以 RRF 合并排名，服务失败时明确回退到关键词。短追问会同时参考上一条用户问题。
...
向量缓存在独立的本地 JSON 文件中，可重启复用和按需补建。当前不包含独立的语义重排模型、PDF / Word 解析、OCR 或远程网页抓取。来源地址是来源说明，不会触发网络下载。语义召回、词项匹配和引用编号校验都不等于自动验证论断是否被文献支持。
